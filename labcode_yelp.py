...
class Recommender:
    ...
    def capsulenet(self, embs, A, num_src, num_tgt, emb0, layer,num_user,num_item):
        ...
        latdim = args.latdim//(args.n_factors//2)
        srcNodes = tf.squeeze(tf.slice(A.indices, [0, 1], [-1, 1]))
        tgtNodes = tf.squeeze(tf.slice(A.indices, [0, 0], [-1, 1]))
        edgeVals = A.values
...
# """---------------------------------------------------------------module of pred---------------------------------------------------------------"""
    ...
    def sampleTrainBatch(self, batIds, beh):
        preSamp = list(np.random.permutation(args.item))
        temLabel = self.trnMats[beh][batIds].toarray()
        batch = len(batIds)
        ...
        while cnt>0:
            ...
            for j in range(args.sampNum):
                ...
                iLocs[cur] = poslocs[j]
                iLocs[cur + temlen // 2] = neglocs[j]
                cur += 1
        return uLocs, iLocs, True
    
    def sampleTrainBatch_Aux(self, batIds, labelMat):
        ...
        for i in range(batch):
            ...
            if sampNum == 0:
                ...
                neglocs = negSamp_aux(temLabel[i], sampNum, args.item)
    ...
    def trainEpoch(self, ep):
        ...
        num = len(sfIds)
        num_aux = len(sfIds_aux)
        steps = int(np.ceil(num / args.batch)) 
        
        if ep % 121 == 0:
            self.iiAdjs_numpy = []
            self.feed_dict={}
            iiAdjs_tmp = ObtainIIMats(self.trnMats, self.predir, 'load')

            for i in range(len(self.iiMats)):
                idx, data, shape = iiAdjs_tmp[i]
                data = data.astype(np.float32)
                c = sp.coo_matrix((data,idx.T),shape)
                idx, data, shape = transToLsts(c,ui=False)
        ...
        for i in range(steps):
            ...
            for beh in range(args.intTypes):
                uLocs, iLocs, flag = self.sampleTrainBatch(batIds, beh)
            ...
            for beh in range(len(self.iiMats)):
                uLocs_aux, iLocs_aux = self.sampleTrainBatch_Aux(batIds_aux, self.iiAdjs_numpy[beh])
